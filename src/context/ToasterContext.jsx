...
const ToasterProvider = ({ children, styles = { position: "top-left" } }) => {
  ...
  const deleteToast = (msg) =>
    setMessages((prev) => prev.filter((m) => m.id !== msg));

  const toaster = useRef({
    toast: (msg) =>
      setMessages((prev) => [
        ...prev,
        { msg, id: Date.now(), state: "neutral" },
      ]),
    success: (msg) =>
      setMessages((prev) => [
        ...prev,
        { msg, id: Date.now(), state: "success" },
      ]),
    error: (msg) =>
      setMessages((prev) => [...prev, { msg, id: Date.now(), state: "error" }]),
  });
...
const Toast = ({ message, ind, deleteToast, styles, duration = 3000 }) => {
  ...
  return (
    <div
      className={`toast ${isBottom ? "enter-bottom" : ""} ${
        width !== 0 ? "" : isBottom ? "leave-bottom" : "leave"
      }`}
...
const ToastContainer = ({ children, styles }) => {
  ...
  return (
    <div
      className={`toaster ${
        children.length ? "toast-container" : ""
      } ${position}`}
    >
      {children}
    </div>
